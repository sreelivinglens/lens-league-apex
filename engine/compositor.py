"""
Apex Audit Card — v12
Fonts bundled in project /fonts/ directory — works on any server including Railway
...
# ── Font paths — bundled in project, works on Railway ────────────────────────
...
def build_card(photo_path, data, out_path):
    ...
    # ── Fonts — all loaded from bundled /fonts/ directory ────────────────────
    f_strip    = fnt(F_BOLD,   18)
    f_score    = fnt(F_BOLD,   52)
    f_tier     = fnt(F_MONO,   16)
    f_asset    = fnt(F_BOLD,   22)
    f_meta     = fnt(F_REG,    16)
    f_tag      = fnt(F_MONO_R, 15)
    f_mod_lbl  = fnt(F_MONO_R, 15)
    f_mod_val  = fnt(F_BOLD,   28)
    f_sec_hdr  = fnt(F_MONO,   15)
    f_sec_body = fnt(F_REG,    17)
    f_byline_h = fnt(F_MONO,   15)
    f_byline_b = fnt(F_REG,    17)
    f_byline_i = fnt(F_BOLD,   17)
    f_badge    = fnt(F_MONO_R, 15)
    ...
    def badge_block_h(badges, label):
    ...
    right_h += badge_block_h(badges_g, "STRENGTHS")
    right_h += badge_block_h(badges_w, "GAPS")
    ...
    draw.text((PAD, 6), "THE LENS LEAGUE", font=f_strip, fill=(200, 168, 75))
    et  = "APEX DDI ENGINE  ·  FULL EVALUATION"
    etb = draw.textbbox((0, 0), et, font=f_strip)
    draw.text((CW - PAD - (etb[2] - etb[0]), 6), et, font=f_strip, fill=(150, 120, 50))
    ...
    draw.text((BX + (BW - (sb[2] - sb[0])) // 2, BY + 22),
              sc, font=f_score, fill=GOLD)
    tier_t = data.get("tier", "").upper()
    tierb  = draw.textbbox((0, 0), tier_t, font=f_tier)
    draw.text((BX + (BW - (tierb[2] - tierb[0])) // 2, BY + 110),
    ...
    draw.text((CW - PAD - (scb[2] - scb[0]), BOT_Y + 6),
              sc_txt, font=f_strip, fill=(150, 120, 50))
    draw.text((PAD, BOT_Y + 6), "APEX DDI ENGINE",
