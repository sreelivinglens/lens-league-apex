...
GENRE_CONTEXT = {
    ...
    'Wildlife': (
        ...
        "BIRDS — PEAK WING MOMENT AND BURST MODE ADJUSTMENT:\n"
        "Modern cameras at 20–30fps make burst-selectable peak wing moments achievable\n"
        "by any photographer who positions correctly and holds the shutter. The technical\n"
        "difficulty is in access, positioning, and exposure — not in the precise release.\n"
        "DM scale for bird peak-wing frames:\n"
        "  Shot on single-frame or 5fps or less, exact peak frame selected: DM 8.0–9.0.\n"
        "  The timing discipline is real — one wrong frame and it is gone.\n"
        "  Shot on 20–30fps burst, best frame selected in post: DM 6.5–7.5.\n"
        "  The peak frame exists in the burst by probability. Selection is still a skill,\n"
        "  but the unrepeatable moment is not the same as a single deliberate release.\n"
        "  If camera data (EXIF, context) shows burst mode: apply the 6.5–7.5 range.\n"
        "  If unknown: default to 7.0–7.5 unless context strongly suggests single-frame.\n"
        "In all cases: DOD still scores the technical execution (sharpness, exposure,\n"
        "feather separation) at full value. The adjustment is DM only.\n"
    ...
    'Nature': (
        ...
        "HARD FLOOR — NAMED EXAMPLES (these are minimums, not ceilings):\n"
        "  A child under cherry blossoms in soft pink light with bokeh background = VD 7.0 MINIMUM.\n"
        "  Autumn leaves with golden light and colour contrast = VD 6.5 MINIMUM.\n"
        "  Single flower in dramatic rim light against dark background = VD 6.5 MINIMUM.\n"
        "  Storm light on a natural scene with strong tonal drama = VD 8.0+.\n"
    ...
    if camera_track == 'mobile':
        ...
        if _effective_genre in MOBILE_EXCLUDED_GENRES:
            ...
            if mw:
                ...
                lines.append('')
                lines.append('CALIBRATION LINE — MANDATORY FOR MOBILE IMAGES:')
                lines.append('The calibration_line field MUST include this transparency note,')
                lines.append('adapted naturally to the image and evaluation:')
                lines.append('"Your evaluation uses Mobile League weights — Decisive Moment')
                lines.append('and Visual Disruption are weighted more heavily than in Camera')
                lines.append('League, recognising that these qualities are harder to achieve')
                lines.append('on a phone. The effort to get this frame is part of what')
                lines.append('this evaluation is measuring."')
                lines.append('Place this after the percentile statement. Plain English.')
                lines.append('NEVER use the word "score" — use "evaluation" throughout.')
                lines.append('')
...
def auto_score(image_path, genre, title, photographer, subject="", location="", sub_genre=None, species_hint="", exif_context="", seasonal_context="", portfolio_summary=None, user_city="", primary_genre="", image_number=1, previous_score=None, previous_audit=None, same_image_rescore=False, masters_by_genre=None):
    ...
    if _pg and _sg and _pg.lower() != _sg.lower():
        ...
        _cross_genre_block = (
            ...
            "HARD LENGTH LIMIT: 2 sentences maximum. Be precise, not expansive.\n"
