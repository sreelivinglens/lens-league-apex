...
def calculate_score(genre, dod, disruption, dm, wonder, aq, ns=''):
    ...
    ns         : Narrative Sufficiency — 'yes', 'not_sure', or 'no' (default '')
    """
    ...
    # NS Bonus: story presence lifts score — applied AFTER all other modifiers
    # YES = meaning transfers without caption (+0.15)
    # NOT SURE = story implied but incomplete (+0.05)
    # NO = subject only, no narrative (+0.00)
    ns_norm = (ns or '').lower().strip().replace(' ', '_')
    if ns_norm in ('yes',):
        raw += 0.15
        checks['ns_bonus'] = 'yes'
        notes.append('NS Bonus: story confirmed (+0.15)')
    elif ns_norm in ('not_sure', 'not sure'):
        raw += 0.05
        checks['ns_bonus'] = 'not_sure'
        notes.append('NS Bonus: story implied (+0.05)')
    else:
        checks['ns_bonus'] = ns_norm or 'no'
