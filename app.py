    ...
    if request.method == 'POST':
        ...
        if api_key:
            try:
                ...
                # ── Sonnet AI gate — runs before Haiku scoring ────────────
                # Blocks AI-generated images before they reach the scoring engine.
                _sonnet_ai_flagged  = False
                _sonnet_ai_reason   = None
                _sonnet_ai_confidence = 1.0
                try:
                    ...
                    if _img_b64:
                        _spayload = {
                            ...
                            'messages': [{
                                ...
                                'content': [
                                    ...
                                    {'type': 'text', 'text': (
                                        'You are a photography contest integrity verifier. '
                                        'Determine if this image was AI-generated (Midjourney, DALL-E, '
                                        'Firefly, Stable Diffusion, etc.) rather than captured by a camera. '
                                        'CRITICAL PRINCIPLE: In real photography, imperfection is normal. '
                                        'AI images are suspicious BECAUSE they look too perfect. '
                                        'A flawlessly perfect wildlife or street image is MORE suspicious, not less. '
                                        'Flag ai_generated=true if ANY of these: '
                                        '(a) Biologically impossible scenes — predator/prey impossible interactions; '
                                        '(b) Image looks like a stock photo or advertisement rather than a real encounter '
                                        '— too composed, too ideal, too cinematic; '
                                        '(c) Lighting too even and flattering across entire frame simultaneously; '
                                        '(d) Animal fur/feather/skin too regular and perfectly rendered '
                                        '— real animals have asymmetric, irregular markings; '
                                        '(e) Water reflections too geometrically clean despite animal movement; '
                                        '(f) No motion blur, no imperfect focus, no ambient inconsistency '
                                        '— real wildlife shots almost always have some imperfection; '
                                        '(g) B&W images with unnaturally perfect tonal distribution '
                                        'and skin textures typical of AI portraiture; '
                                        '(h) Overall aesthetic resembles AI image generation style. '
                                        'Be sceptical of perfection. Real wildlife photography is hard '
                                        'and the results show it. '
                                        'Respond ONLY with JSON: '
                                        '{"ai_generated":bool,"confidence":0.0-1.0,"reason":"max 30 words"}'
                                    )}
                                ]
                            }]
                        }
                        _sreq = _sur.Request(
                            ...
                            data=_sjson.dumps(_spayload).encode('utf-8'),
                            headers={
                                'Content-Type': 'application/json',
                                'x-api-key': api_key,
                        ...
                        with _sur.urlopen(_sreq, timeout=20) as _sresp:
                            _srdata = _sjson.loads(_sresp.read().decode())
                            _stext  = _srdata.get('content', [{}])[0].get('text', '{}')
                            _stext  = _stext.strip().lstrip('`').lstrip('json').strip('`').strip()
                            _sresult = _sjson.loads(_stext)
                        _sonnet_ai_flagged   = bool(_sresult.get('ai_generated', False))
                        _sonnet_ai_confidence = float(_sresult.get('confidence', 1.0))
                        _sonnet_ai_reason    = _sresult.get('reason', '')
                        app.logger.info(
                            f'[sonnet_gate] ai={_sonnet_ai_flagged} '
                            f'confidence={_sonnet_ai_confidence} reason={_sonnet_ai_reason}'
                        )
                except Exception as _sgate_err:
                    app.logger.warning(f'[sonnet_gate] failed (non-fatal): {_sgate_err}')

                # If Sonnet flags AI → block immediately, skip Haiku scoring
                if _sonnet_ai_flagged and _sonnet_ai_confidence >= 0.8:
                    img.score            = 0.0
                    img.tier             = 'Rookie'
                    img.dod_score        = 0.0
                    ...
                    img.needs_review     = True
                    img.is_public        = False
                    img.ai_suspicion     = _sonnet_ai_confidence
                    img.ai_suspicion_reason = f'Sonnet gate: {_sonnet_ai_reason}'
                    img.flagged_reason   = f'AI generation detected by integrity check. {_sonnet_ai_reason}'.strip()
                    ...
                    try:
                        ...
                        send_email(
                            ...
                            html_body=('<p>Hi ' + _uname + ',</p><p>Your image <strong>' +
                                (img.asset_name or 'Untitled') +
                                '</strong> has been flagged as AI-generated and cannot be submitted.</p>'
                                '<p>Only original photographs taken by you on your camera are accepted.</p>'
                                '<p>Contact <a href="mailto:' + CONTACT_EMAIL + '">' + CONTACT_EMAIL +
                                '</a> if you believe this is an error.</p>'
                                '<p>The Shutter League Team</p>'),
                            text_body=('Hi ' + _uname + ', Your image ' +
                                (img.asset_name or 'Untitled') +
                                ' was flagged as AI-generated. Contact ' +
                                CONTACT_EMAIL + ' if this is an error. The Shutter League Team')
                        )
                        send_email(
                            to_addresses=[ADMIN_EMAIL],
                            subject='[Admin] Sonnet AI Gate — ' + (img.asset_name or 'Untitled'),
                            html_body=('<p>AI image blocked at Sonnet gate.</p><ul>'
                                '<li>Image: ' + (img.asset_name or 'Untitled') + '</li>'
                                '<li>Confidence: ' + str(round(_sonnet_ai_confidence, 2)) + '</li>'
                                '<li>Reason: ' + (_sonnet_ai_reason or '') + '</li>'
                                '<li>User: ' + (_u.email if _u else 'unknown') + '</li></ul>'),
                            text_body=('Sonnet AI Gate. Image: ' + (img.asset_name or 'Untitled') +
                                ' Confidence: ' + str(round(_sonnet_ai_confidence, 2)) +
                                ' Reason: ' + (_sonnet_ai_reason or '') +
                                ' User: ' + (_u.email if _u else 'unknown'))
                        )
                    except Exception as _sme:
                        app.logger.error(f'[sonnet_gate email error] {_sme}')
                    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                        return jsonify({
                            'error': True,
                            'message': (
                                ' This image has been flagged as AI-generated and cannot be submitted. '
                    ...
                    if os.path.exists(img.thumb_path or ''): os.remove(img.thumb_path)
                    return redirect(request.url)

                # Sonnet passed (or gate failed non-fatally) — proceed to Haiku scoring
