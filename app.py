...
def send_email(to_addresses, subject, html_body, text_body=None, attachments=None):
    """
    ...
    attachments: optional list of local file paths to attach (e.g. sample
                 scorecard JPG). Each file is read and base64-encoded into
                 the Brevo payload. Missing files are skipped with a warning
    ...
    if attachments:
        ...
        for file_path in attachments:
            try:
                ...
                app.logger.warning(f'[email] Could not attach {file_path}: {_attach_err}')
        if attachment_list:
...
def auth_google_callback():
    try:
        ...
        if 'mismatching_state' in err or 'MismatchingState' in err or 'state' in err.lower():
            flash('Sign-in interrupted — please try signing in again.', 'error')
        else:
...
def mission():
    ...
    # ?indoor=portraits or ?indoor=stilllife arrives from the dashboard red-weather card.
    # Replaces the outdoor assignment with the lesson's indoor_assignment text.
    _indoor = request.args.get('indoor') or None
    if _indoor and lesson and lesson.get('indoor_assignment'):
...
def dashboard():
    ...
    if current_user.role != 'admin':
        ...
        _mission_done = False
        try:
            # IST midnight, expressed back in UTC for comparison against created_at
            # (created_at is stored in UTC). Using utcnow().replace(hour=0) directly
            # anchors "today" to UTC midnight (05:30 IST) — between 12:00 AM and
            # 5:30 AM IST, a new IST day has begun but UTC has not rolled over yet,
            # so the old boundary stretched back into *yesterday* (IST), letting a
            # mission completed last night show as stale "done" carryover into the
    ...
    # Hero carousel for dashboard banner — random high-scoring public image per visit
    # Same query as index.html carousel but random order so it changes on every refresh
    try:
        _dash_carousel = (Image.query
                          .filter(Image.status=='scored', Image.score!=None,
                                  Image.is_public==True, Image.is_flagged==False,
                                  Image.tier.in_(['Legend','Grandmaster','Master']),
                                  Image.score>=8.5,
...
@login_required
def my_gallery():
    """User's complete image history — all uploads, with scores, edit/delete access."""
    page    = request.args.get('page', 1, type=int)
    query   = request.args.get('q', '').strip()
    ...
    if sort == 'score_desc':
        ...
        images_q = images_q.filter(Image.score != None).order_by(Image.score.asc())
    elif sort == 'oldest':
        images_q = images_q.order_by(Image.created_at.asc())
    else:  # newest
        images_q = images_q.order_by(Image.created_at.desc())
...
# Photo School curriculum data — see curriculum_data.py (extracted Session 95)
from curriculum_data import _CURRICULUM, _CURRICULUM_BY_ID
                    ...
                    try:
                                    ...
                                    try:
                                        ...
                                        send_email(
                                            ...
                                            html_body=(
                                                ...
                                                'photographer.</p>'
                                                '<p style="font-size:16px;line-height:1.7;color:#111111;">'
                                                'High-production constructed shoots — allegorical tableaux, '
                                                'studio work, heavily lit portraits — can occasionally '
                                                'trigger false positives. If this is an original photograph, '
                                                'we want to clear it quickly.</p>'
                                                '<p style="font-size:16px;line-height:1.7;color:#111111;">'
                                                '<strong>To appeal, email '
                                                '<a href="mailto:' + CONTACT_EMAIL + '">'
                                                + CONTACT_EMAIL + '</a> with:</strong></p>'
                                                '<ul style="font-size:16px;line-height:2;color:#111111;">'
                        ...
                        try:
                            ...
                            if not _sc_seasonal_ctx and _sc_user_city and (_sc_primary_genre or _img.genre):
                                ...
                                try:
                                    from engine.seasonal_discovery import discover_one, enqueue_priority_combo
                            ...
                            try:
                                ...
                                send_email(
                                    ...
                                    html_body=(
                                        ...
                                        '<p style="font-size:16px;line-height:1.7;color:#111111;">We know that high-production constructed shoots — studio work, allegorical tableaux, heavily lit portraits — can sometimes trigger false positives. If this is an original photograph, we want to clear it quickly.</p>'
                                        '<p style="font-size:16px;line-height:1.7;color:#111111;"><strong>To appeal, email <a href="mailto:' + CONTACT_EMAIL + '">' + CONTACT_EMAIL + '</a> with:</strong></p>'
                                        '<ul style="font-size:16px;line-height:2;color:#111111;">'
                                        '<li>Your original RAW or unedited file</li>'
                                        '<li>Any behind-the-scenes photos, casting, or production notes (if available)</li>'
                                        '<li>Camera and lens used</li>'
                                        '</ul>'
                                    ...
                                    text_body=(
                                        'Hi ' + _uname + ',\n\n'
                                        'Your image "' + (_img.asset_name or 'Untitled') + '" has been flagged for admin review before going public.\n\n'
                                        'Our automated system detected characteristics that may indicate AI generation. '
                                        'High-production constructed shoots can sometimes trigger false positives — if this is an original photograph, we want to clear it quickly.\n\n'
                                        'To appeal, email ' + CONTACT_EMAIL + ' with:\n'
                                        '  - Your original RAW or unedited file\n'
                                        '  - Any behind-the-scenes photos or production notes (if available)\n'
                                        '  - Camera and lens used\n\n'
                                        'We aim to resolve appeals within 48 hours.\n\n'
                                        '-- Shutter League'
                                    )
...
@login_required
@admin_required
def admin_force_rescore(image_id):
    """
    Admin-only: force a full rescore of a SINGLE already-scored image using
    ...
    Use case: an image was scored before a calibration/prompt fix and needs
    to be re-evaluated under the corrected rules (e.g. Latmarr Holi /
...
def _force_rescore_in_background(image_id, old_score, old_tier, old_status='scored'):
    ...
    with app.app_context():
        ...
        try:
            ...
            # On-demand discovery — same as upload path: call discover_one()
            # directly for this specific city/genre (cheaper than
            # run_seasonal_discovery which scans all users first).
...
def learning():
    ...
    # Build mentor list respecting is_active — same logic as /mentors route
    mentors_data = {}
...
def send_welcome_email(user):
    ...
    # Fixed, founder-approved real imagery (see note above on why these are
    # static rather than a live random query).
    hero_thumb_url     = 'https://pub-1b176cd1cfcc4e699e024f0907bef610.r2.dev/thumbs/b93bb08b-9314-421c-a449-f3e6393b72be.jpg'
    hero_score         = '8.84'
    hero_tier          = 'Master'
    hero_genre         = 'Street'
    hero_photographer  = 'Ashok Kochhar'
    ...
    text_body = (
        ...
        'WE SEE YOU.\n'
        'Your photograph captured something your eye saw. Most people never get told that part.\n\n'

        'Chess has a rating. Cricket has a standing. Michelin has stars. F1 has points. '
        'But for 200 years, since the first photograph was taken, photography never got one.\n'
        '"Photographers have received two kinds of feedback: flattery and silence."\n'
        'Likes are not feedback. Evaluations without reasons are not feedback. '
        'Feedback is what works, what doesn\'t, and what to improve next.\n'
        'Photography has never had an honest improvement loop. Until now.\n\n'
        ...
        'shoot, your city, and the part of your eye that\'s ready to grow next.\n'
        'See today\'s assignment: ' + dashboard_url + '\n\n'

        'POINTS\n'
        'Every evaluation you receive earns you points. Rate someone else\'s work and you earn '
        'more - the more photographers give feedback, the sharper everyone\'s eye gets, yours included.\n'
        'Save them for extra uploads, or trade them for real time with a mentor - from 1,000 points.\n'
        'Your wallet: ' + dashboard_url + '\n\n'

        'LEARN FROM MASTERS\n'
        'Ashok Kochhar, Legend Mentor. A real photographer reads your frame - not a template.\n'
        'Meet the mentors: ' + mentors_url + '\n\n'

        'YOUR STANDING\n'
        'Every evaluation adds to something larger - your standing toward the Annual Excellence '
        ...
        'WHAT AWAITS YOU\n'
        'Weekly Assignment - one theme, one week, a low-stakes place to try the thing you\'ve '
        'been putting off.\n' + challenge_url + '\n\n'
        ...
        '- Minimum 1500px on the short edge.\n'
        '- Mobile League: smartphone only. Camera League: DSLR or mirrorless only.\n'
        '- Keep your RAW files. May be requested for verification.\n\n'

        'Upload your first photograph today. Tomorrow morning, your first assignment will be waiting.\n'
